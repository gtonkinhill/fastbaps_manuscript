...
system.time({hiv.multi <- multi_res_baps(hiv.data, levels = 10, n.cores = 4, k.init = 11000)})

write.csv(hiv.multi, file="hiv_multi_opt_symmetric_prior.csv", quote=FALSE)
```

```{r, eval=FALSE}
system.time({hiv.data <- optimise_prior(hiv.data, type = "optimise.baps", n.cores = 10)})

system.time({hiv.multi <- multi_res_baps(hiv.data, levels = 2, n.cores = 10, k.init = 11000)})

write.csv(hiv.multi, file="hiv_multi_opt_baps_prior.csv", quote=FALSE)
```

```{r}
hiv.multi.opt.baps <- fread("./processed_data/hiv_multi_opt_baps_prior.csv", data.table = FALSE)

hiv.multi.opt.symmetric <- fread("./processed_data/hiv_multi_opt_symmetric_prior.csv", data.table = FALSE)
```
...
colnames(hiv.multi.merged) <- c("Isolate", "Level 1.sym", "Level 2.sym", "Level 1.baps", "Level 2.baps")

hiv.multi.merged <- merge(hiv.multi.merged, best.baps.partition, by.x = "Isolate", by.y="Isolates")

write.table(hiv.multi.merged, "./processed_data/hiv_multi_merged.csv", sep=",", col.names = TRUE, row.names = FALSE, quote = FALSE)
```


```{r}
hiv.multi.merged <- fread("./processed_data/hiv_multi_merged.csv", header = TRUE, data.table = FALSE)
...
k.n.clusters <- max(hiv.multi.merged$`Level 1.baps`)
k.191 <- kmeans(pc$x, centers = k.n.clusters, iter.max = 50, nstart = 5)$cluster
names(k.191) <- colnames(hiv.data$snp.matrix)

hiv.multi.merged$kmeans.191 <- k.191[match(hiv.multi.merged$Isolate, names(k.191))]

all.k <- lapply(1:200, function(k){
  k <- kmeans(pc$x, centers = k, iter.max = 50, nstart = 5)
})
...
hiv.multi.merged$kmeans.17 <- elbow.k[match(hiv.multi.merged$Isolate, names(elbow.k))]
```

```{r, eval=FALSE}
# hc.genie <- genie::hclust2(d="euclidean", objects = pc$x, useVpTree=FALSE, thresholdGini = 0.2)
# hc.genie$labels <- colnames(hiv.data$snp.matrix)
# hc.genie.bbp <- fastbaps::best_baps_partition(hiv.data, hc.genie)
# 
# write.table(data.frame(Isolate=names(hc.genie.bbp), hc.genie.partition=hc.genie.bbp, stringsAsFactors = FALSE),
#             file = "./processed_data/hcgenie_best_baps_partition.csv", sep=",", col.names = TRUE, row.names = FALSE, quote = FALSE)
```

```{r}
# hclust.genie.bbp <- fread("./processed_data/hcgenie_best_baps_partition.csv",
#                           data.table = FALSE)
# hiv.multi.merged <- merge(hiv.multi.merged, hclust.genie.bbp, by.x="Isolate", by.y = "Isolate")
```
...
par(mfrow=c(1,2))
plot(tree.rooted, show.tip.label=FALSE)
plot(x = hiv.multi.merged$hc.genie.partition, 
     y = c(1:nrow(hiv.multi.merged)), 
     pch=0.1, cex=0.2, axes = FALSE)
par(mfrow=c(1,1))
```
...
```{r}
combined.df <- combined.df[,c(1,2,3,4,5,7,9,10)]
colnames(combined.df) <- c("Isolate","Type","Country","Date",
...
# all.method.clust.props <- all.method.clust.props[!(all.method.clust.props$method %in% c("Level 1.sym", "Level 1.baps")),]
...
combined.df$k.means.17 <- elbow.k

plot.df <- data.frame(Isolates = colnames(hiv.data$snp.matrix), hiv.um$layout, 
                      k.means.191=combined.df$`kmeans k=191`,
                      k.means.17=combined.df$k.means.17,
                      stringsAsFactors = FALSE)
...
plot(X2~X1, data=plot.df, col=k.means.191, cex=0.1,
     xlab="", ylab="", 
     cex.axis=1.5, font=1.5)
mtext(side=1, line=3, "UMAP DIM 1", cex=1.5)
mtext(side=2, line=3, "UMAP DIM 2", cex=1.5)

plot(X2~X1, data=plot.df, col=k.means.17, cex=0.1,
     xlab="", ylab="", 
     cex.axis=1.5, font=1.5)
mtext(side=1, line=3, "UMAP DIM 1", cex=1.5)
mtext(side=2, line=3, "UMAP DIM 2", cex=1.5)
...
mtext(side=1, line=3, "UMAP DIM 1", cex=1.5)
mtext(side=2, line=3, "UMAP DIM 2", cex=1.5)
text(x =plot.df$X1, y=plot.df$X2, labels = plot.df$k.means.191, col=plot.df$k.means.191, cex=0.3)
...
mtext(side=1, line=3, "UMAP DIM 1", cex=1.5)
mtext(side=2, line=3, "UMAP DIM 2", cex=1.5)
text(x =plot.df$X1, y=plot.df$X2, labels = plot.df$k.means.17, col=plot.df$k.means.17, cex=0.3)

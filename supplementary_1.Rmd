...
# d <- snp_dist(h.inf.data)
# h <- hclust(as.dist(d), method="ward.D2")
# h.inf.multi.hc <- multi_level_best_baps_partition(h.inf.data, h, levels = 2, n.cores = 4)
...
```{r}
function(sparse.data, k.init=NULL, n.replicates=100, n.cores=1, quiet=TRUE){
...
# d <- snp_dist(listeria.data)
# h <- hclust(as.dist(d), method="ward.D2")
# listeria.multi.hc <- multi_level_best_baps_partition(listeria.data, h, levels = 2, n.cores = 4)
...
# d <- snp_dist(n.meng.data)
# h <- hclust(as.dist(d), method="ward.D2")
# n.meng.multi.hc <- multi_level_best_baps_partition(n.meng.data, h, levels = 2, n.cores = 4)
...
# d <- snp_dist(s.aus.data)
# h <- hclust(as.dist(d), method="ward.D2")
# s.aus.multi.hc <- multi_level_best_baps_partition(s.aus.data, h, levels = 2, n.cores = 4)
...
names(k) <- colnames(hiv.data$snp.matrix)

hiv.multi.merged$kmeans.191 <- k[match(hiv.multi.merged$Isolate, names(k))]

k.n.clusters <- max(hiv.multi.merged$best.baps.partition)
k <- kmeans(pc$x, centers = k.n.clusters, iter.max = 50, nstart = 5)$cluster
names(k) <- colnames(hiv.data$snp.matrix)

hiv.multi.merged$kmeans.368 <- k[match(hiv.multi.merged$Isolate, names(k))]
```
...
cluster.types <- lapply(1:max(combined.df$best.baps.partition), function(x){
  tb <- table(combined.df$Type[combined.df$best.baps.partition==x])
  tb[order(tb)]
})
...
```{r}
colnames(combined.df) <- c("Isolate","Type","Country","Date",
                           "Level 1 optimised symmetric prior", "Level 2 optimised symmetric prior",
                           "Level 1 optimised BAPs prior","Level 2 optimised BAPs prior",
                           "Best BAPs partition fasttree",
                           "kmeans k=191", "kmeans k=368",
                           "Best BAPs partition Genie")
...
Look at projection using UMAP

```{r}
library(umap)

um <- umap::umap(pc$x, method="umap-learn")

plot(um$layout[,1], um$layout[,2], col=factor(combined.df$`Level 1 optimised BAPs prior`), pch=1)

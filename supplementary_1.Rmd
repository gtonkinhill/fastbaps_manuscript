...
all.k <- parallel::mclapply(1:200, function(k){
...
hiv.multi.merged$kmeans.18 <- elbow.k[match(hiv.multi.merged$Isolate, names(elbow.k))]
...
```{r}
combined.df <- combined.df[,c("Isolate", "Type", "Country", "Date", "Level 1.sym", "Level 1.baps", "best.baps.partition","kmeans.18", "kmeans.193")]
colnames(combined.df) <- c("Isolate","Type","Country","Date",
...
all.method.clust.props <- all.method.clust.props[!(all.method.clust.props$method %in% c("kmeans k=18")),]
...
plot(X2~X1, data=plot.df, col=k.means.18, cex=0.1,
     xlab="", ylab="", 
     cex.axis=1.5, font=1.5)
mtext(side=1, line=3, "UMAP DIM 1", cex=1.5)
mtext(side=2, line=3, "UMAP DIM 2", cex=1.5)
...
mtext(side=1, line=3, "UMAP DIM 1", cex=1.5)
mtext(side=2, line=3, "UMAP DIM 2", cex=1.5)
text(x =plot.df$X1, y=plot.df$X2, labels = plot.df$k.means.18, col=plot.df$k.means.18, cex=0.3)

...
```{r, eval=FALSE}
system.time({hiv.data <- optimise_prior(hiv.data, type = "symmetric", n.cores = 4)})
...
```{r, eval=FALSE}
system.time({hiv.data <- optimise_prior(hiv.data, type = "optimise.baps", n.cores = 4)})
...
hiv.multi.merged$best.baps.partition.tree <- best.baps.partition[match(hiv.multi.merged$Isolate, names(best.baps.partition))]
...
plot(x = hiv.multi.merged$best.baps.partition.tree, 
...
cluster.types <- lapply(1:max(combined.df$best.baps.partition.tree), function(x){
  tb <- table(combined.df$Type[combined.df$best.baps.partition.tree==x])
  tb[order(tb)]
})
cluster.types[order(unlist(lapply(cluster.types, sum)))]

cluster.types <- lapply(1:max(combined.df$`Level 2.baps`), function(x){
  tb <- table(combined.df$Type[combined.df$`Level 2.baps`==x])
  tb[order(tb)]
})
names(cluster.types) <- 1:max(combined.df$`Level 2.baps`)
cluster.types[order(unlist(lapply(cluster.types, sum)))]
...
# tiplabels(pch=1,
#     col = hiv.multi.opt.symmetric$`Level 1`)
plot(x = hiv.multi.merged$`Level 2.baps`[hiv.multi.merged$`Level 2.baps`==87], 
     y = c(1:nrow(hiv.multi.merged))[hiv.multi.merged$`Level 2.baps`==87], 
     pch=0.1, cex=0.2, axes = FALSE)
par(mfrow=c(1,1))

ggplot(combined.df, aes(x=Type)) + geom_bar() + facet_wrap(~`Level 2`, scales = "free", ncol = 5)
```
...
cluster.stats <- data.frame(cluster = names(props),
                            prop.match.top = props,
                            `cluster size` = clust.sizes, stringsAsFactors = FALSE)

ggplot(cluster.stats, aes(x=1, y=prop.match.top)) + geom_violin() +
...
names(cluster.types) <- 1:max(combined.df$best.baps.partition.tree)
cluster.types <- cluster.types[order(unlist(lapply(cluster.types, sum)))]

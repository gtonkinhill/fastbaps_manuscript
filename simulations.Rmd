...
simulate_island_inputs <- function(n.pops, pop.size, seq.length,
  ...
  while(ncol(simu.stats$segsites[[1]]$snps)<2){
    simu.stats <- simulate(model)
  }
...
mutation.rates <- c(0.0001, 0.001, 0.01)
...
results.df <- do.call(rbind, results.df)
...
# dim(parameters.df)
# 
# results2.df <- do.call(rbind, mclapply(1:nrow(parameters.df), function(i){
#   print(i)
#   params <- parameters.df[i,,drop=FALSE]
...
# },  mc.cores = CPU))
# 
# write.table(results2.df, file="./simulations/simulation_results_island_no_structure.csv",
#             row.names = FALSE, col.names = TRUE, quote=FALSE, sep=",")
```
